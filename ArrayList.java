...
public class ArrayList<T> {
    ...
    public void addAtIndex(int index, T data) {
         ...
         if (size == backingArray.length) {
            T[] dupArray = (T[]) new Object[size * 2];
            for (int i = 0; i < backingArray.length; i++) {
                dupArray[i] = backingArray[i];
            }
            backingArray = dupArray;
         }
         if (index != size) {
     ...
     */
    public void clear() {
        T[] cleared = (T[]) new Object[INITIAL_CAPACITY];
        backingArray = cleared;
        size = 0;
    }
